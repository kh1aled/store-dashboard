...
require_permission($pdo, 'manage_products');
$active = 'products';
$id = (int)($_GET['id'] ?? 0);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($data['sku'] === '' || $data['name'] === '' || $data['selling_price'] < 0 || $data['stock'] < 0 || $data['discount'] < 0 || $data['discount'] > 100) $error = 'SKU, name and valid pricing/stock are required.';
        ...
        try {
            $sql = $id ? "UPDATE products SET sku=?,name=?,description=?,cost_price=?,selling_price=?,discount=?,stock=?,category_id=?,brand_id=?,partner_id=?,image=?,status=? WHERE id=?" : "INSERT INTO products(sku,name,description,cost_price,selling_price,discount,stock,category_id,brand_id,partner_id,image,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
            ...
            $error = 'Could not save product. SKU must be unique.';
        }
...
<div class="mb-4"><span class="eyebrow">CATALOG</span>
    <h1 class="page-title"><?= $id ? 'Edit' : 'Add' ?> Product</h1>
</div>
<div class="panel form-panel"><?php if ($error): ?><div class="alert alert-danger"><?= e($error) ?></div><?php endif; ?><form method="post"><?= csrf_field() ?><div class="row g-3">
            ...
            <div class="col-md-4"><label class="form-label">Category</label><select class="form-select" name="category_id">
                    <option value="">—</option><?php foreach ($cats as $x): ?><option value="<?= $x['id'] ?>" <?= $row['category_id'] == $x['id'] ? 'selected' : '' ?>><?= e($x['name']) ?></option><?php endforeach; ?>
                </select></div>
            <div class="col-md-4"><label class="form-label">Brand</label><select class="form-select" name="brand_id">
                    <option value="">—</option><?php foreach ($brands as $x): ?><option value="<?= $x['id'] ?>" <?= $row['brand_id'] == $x['id'] ? 'selected' : '' ?>><?= e($x['name']) ?></option><?php endforeach; ?>
                </select></div>
            <div class="col-md-4"><label class="form-label">Partner</label><select class="form-select" name="partner_id">
                    <option value="">—</option><?php foreach ($partners as $x): ?><option value="<?= $x['id'] ?>" <?= $row['partner_id'] == $x['id'] ? 'selected' : '' ?>><?= e($x['name']) ?></option><?php endforeach; ?>
                </select></div>
        ...
        <div class="mt-4"><button class="btn btn-stare">Save Product</button> <a class="btn btn-outline-secondary" href="index.php">Cancel</a></div>
    </form>
</div>
